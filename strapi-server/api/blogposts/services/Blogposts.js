...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Blogposts.query(function(qb) {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Blogposts._attributes)
      .filter(attribute => attribute !== Blogposts.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Blogposts._attributes[attribute].type));
